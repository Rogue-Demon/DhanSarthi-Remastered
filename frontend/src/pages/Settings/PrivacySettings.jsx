...
function ToggleCard({ title, desc, icon, enabled, onToggle }) {
  ...
  return (
    <div className="clay-surface bg-card p-5 border border-white/60 dark:border-white/5 rounded-2xl shadow-xs flex items-center justify-between gap-4">
      ...
      <button
        onClick={onToggle}
        className={`w-12 h-6 rounded-full transition-colors relative p-1 cursor-pointer shrink-0 ${enabled ? 'bg-primary' : 'bg-muted'}`}
      >
        <div
          className={`h-4 w-4 rounded-full bg-white transition-transform ${enabled ? 'translate-x-6' : 'translate-x-0'}`}
        />
      </button>
    </div>
  )
    ...
    >
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs font-bold text-text-muted">
          Manage data sharing, analytics collection, and cookie policies.
        </p>
      </div>

      <ToggleCard
        title="Anonymous Data Sharing"
        desc="Share anonymised usage patterns to improve DhanSarthi."
        icon="Share2"
        enabled={dataSharing}
      ...
      <ToggleCard
        ...
        desc="Allow internal usage analytics for performance monitoring."
        icon="BarChart"
        enabled={analytics}
      ...
      <ToggleCard
        ...
        desc="Enable functional and analytics browser cookies."
        icon="Cookie"
        enabled={cookies}
